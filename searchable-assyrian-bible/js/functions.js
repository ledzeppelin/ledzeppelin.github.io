function loadResults(searchQuery, PAGINATE_AMT) {
  ...
  searchQuery.results.slice(searchQuery.i, searchQuery.i + PAGINATE_AMT).forEach((result) => {
    ...
    const biblePromotionContainer = $(document.createDocumentFragment());
    ...
    if (isFirstResult) {
      biblePromotionContainer.append(
        $('<div/>', { id: 'bible-promotion-container' }).append(
          $('<span/>', {
            class: 'webapp-backlink-meta',
            text: 'results from 🔵 ',
          }),
          $('<a/>', {
            class: 'webapp-backlink-href',
            text: 'assyrian bible',
            href: '../assyrian-bible',
          }),
        ),
