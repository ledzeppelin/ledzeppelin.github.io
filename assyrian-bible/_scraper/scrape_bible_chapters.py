...
class TestSpider(scrapy.Spider):
    ...
    for book_chapters in books_chapters:
        ...
        for chapter in range(1, chapters + 1):
            for translation, code in translations.items():
                ...
                start_url = (
                    f"https://www.bible.com/bible/{code}/{chapter_of_translation}"
                )
    ...
    def parse(self, response):
        ...
        children = response.css(
            f'div[data-usfm="{chapter_name}.{chapter_number}"] > div'
        )
        for child in children:
            heading = (
                child.css(".s1 .heading::text").get()
                or child.css(".ms .heading::text").get()
                or child.css(".qa .heading::text").get()
            )
            reference = (
                child.css(".r .heading::text").get()
                or child.css(".mr .heading::text").get()
            )
            ...
            if reference is not None and translation == "AII":
                ...
                for ref_replacement in ref_replacements:
                    ...
                    if ref_replacement in reference:
                        if any(
                            ref_replacement in prev_replacement
                            for prev_replacement in prev_replacements
                        ):
                            continue
                        ...
                        with tag(
                            "a",
                            klass="bible-backlink",
                            href=f"?book={book}&chapter={chap}:{start}-{end}",
                        ):
                ...
                while True:
                    ...
                    verse_data = parse_verse(
                        verse_key, verse_num, verse, next_verse, response
                    )
...
def parse_verse(verse_key, verse_num, verse, next_verse, response):
    ...
    if verse is not None:
        ...
        sentence = (
            sentence.replace("ܚܲܕܘܼ", "1111111111")
        ...
        sentence = (
            sentence.replace("1111111111", "ܚܲܕܘܼ")
