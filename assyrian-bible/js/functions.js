  ...
  bible[bookShortName][chapter - 1].forEach((verseSection, i) => {
    ...
    verseSection.v.forEach((verse, j) => {
      ...
      if (readingLevel === 1) {
        ...
        if (i === 0 && j === 0 && verse[0] != null) {
          const bibleCaption = $('<div/>', {
            class: 'translation-caption',
            text: '^ not a word-for-word english translation of the assyrian bible',
          });

          engVerse = engLi.wrapInner('<div>').append(bibleCaption);
        } else {
          engVerse = engLi;
        }
      } else {
        engVerse = ''; // use empty string instead of null https://stackoverflow.com/a/22657035
      }
