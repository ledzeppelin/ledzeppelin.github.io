...
def set_examples(item_sense, sense):
    if "examples" in item_sense:
        ...
        for example in item_sense["examples"]:
            if "type" in example and example["type"] not in {
                "example",
                "quotation",
                "quote",
            }:
...
def set_infl(item, obj, aii_v, verb_denominal_forms, dynamic_noun_forms, NOUN_INFL_CNT):
    ...
    if html_template_name == "aii-conj":
        ...
        call_site = {"name": leaf_template["name"], "1": leaf_template["args"].pop("1")}
...
def set_noun_infl(item, obj, template_name):
    ...
    obj["table"] = {"heading": noun_infl_schema[template_name]["heading"], "rows": rows}
...
def set_head_template(item, obj, aii_v):
    ...
    for arg, val in item["head_templates"][0]["args"].items():
        if arg in ht_schema[ht_name]["omit"]:
            ...
            obj["other_forms"]["rows"].append(annotated_row(form_name, [val]))
            ...
            raise Exception(
                f'"{arg}" not in omit, forms or genders for ht {ht_name} for:\n\n {item}'
            )
...
def set_linkage_table(parent, obj, aii_v):
    ...
    for (
        linkage_key,
        linkage_val,
    ) in consts.linkage_types.items():  # ex. {'alt_of': 'alternate',}
    ...
    if "sounds" in item:
        for sound in item["sounds"]:
            ...
            if "(" in ipa_str or ")" in ipa_str:
                urls_with_ipa_parens.add(f"https://en.wiktionary.org/wiki/{aii_not_v}")
    ...
    for aii_not_v, aii_vs in aii_sounds.items():
        for aii_v, jsonlines_ipas in aii_vs.items():
            ...
            if collapsed:
                ...
                unique_ipas_lst = [
                    (sorted(v), k[0], k[1]) for k, v in unique_ipas.items()
                ]
...
def annotate_row_if_not_there(meta, aii_values, obj):
    ...
    if aii_values:
        obj["other_forms"]["rows"].append(annotated_row(meta, aii_values))
...
def add_other_forms_from_ety_templates(ety, obj, item, aii_v, alias):
    if ety["name"] == "cog" and "2" in ety["args"]:
        ...
        ety["name"] in ("contraction")
        and ety["args"]["1"] == "aii"
        and "2" in ety["args"]
    ):
        annotate_row_if_not_there(alias, [ety["args"]["2"]], obj)
    elif (
        ety["name"] in ("m", "l", "clipping", "back-form") and ety["args"]["1"] == "aii"
    ):
        annotate_row_if_not_there(alias, [ety["args"]["2"]], obj)
        ...
        for doublet_num in ["2", "3", "4"]:
            if doublet_num in ety["args"]:
                annotate_row_if_not_there(
                    alias, [remove_inline_modifier(ety["args"][doublet_num])], obj
                )
    elif (
        ety["name"]
        in ("affix", "af", "com", "compound", "surf", "blend", "univerbation")
        and ety["args"]["1"] == "aii"
    ):
...
def set_duplicate_sounds(item, aii_not_v, duplicate_sounds_urls):
    ...
    if isinstance(sounds, list):
        standard_count = sum(
            1 for s in sounds if isinstance(s, dict) and s.get("tags") == ["Standard"]
        )
        if standard_count > 1:
...
def datadump_to_dict():
    ...
    for item in data:
        ...
        set_infl(
            item, obj, aii_v, verb_denominal_forms, dynamic_noun_forms, NOUN_INFL_CNT
        )
