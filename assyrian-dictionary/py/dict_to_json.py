    ...
    if should_collapse_etys:
        ...
        for jsonline in jsonlines:
            if "tier2_etymology" in jsonline:
                jsonline["tier2_tags"].extend(
                    [f"from:{language}" for language in jsonline["tier2_etymology"]]
                )
    ...
    for jsonlines in aii_v_s.values():
        for jsonline in jsonlines:
            if jsonline["pos"] in {f"{i}-letter root" for i in range(3, 6)}:
                if len(aii_v_s) > 1:
                    raise Exception(
                        "numerous vocalized spellings for the unvocalized root"
                    )
                if len(jsonlines) > 1:
                    raise Exception(
                        "numerous jsonlines/parts of speech for the unvocalized root"
                    )
                return True
    return False
...
def aii_dict_to_fuse(aii_dict, sounds, numbers_table):
    ...
    if NUM_OMIT_T2_CATS[0] < 0:
        raise Exception(
            f"increase NUM_OMIT_T2_CATS threshold {UNFILTERED_WORDS_WITH_T2_CATS}"
        )
